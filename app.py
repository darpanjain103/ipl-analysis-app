...
if fetch_data:
    ...
    def make_group_table(df, group_by_col, display_name=None):
        ...
        runs_balls = runs_balls_df.groupby(group_by_col).agg(
            ...
            Control=("battingConnectionId", lambda x: x.fillna('None').isin(['Left','Middled','WellTimed','None', 'Played', 'Padded', 'ShoulderArms']).sum())
        ).reset_index()
    ...
    if batting_players:
        ...
        with tab10:
            shot_area_df = filtered_df[~filtered_df["fieldingPosition"].fillna("").str.strip().isin(["", "-"])]
